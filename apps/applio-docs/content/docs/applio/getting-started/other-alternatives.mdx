...
The notebook is easy to understand, even more so when this notebook shares the same processes as the UI version!, so you can be guided by the existing explanations such as [Training](/docs/getting-started/training) or [Inference.](/docs/getting-started/download-models)
...
- Download the [pretrained](/docs/getting-started/pretrained#where-to-find-pretraineds) you want to use and upload the d.pth and g.pth files to your Google Drive.
...
- Finally, copy the path of G.pth and paste it into **g_pretrained_path**, and do the same for D.pth into **d_pretrained_path**. It should look like this: `/content/Applio/rvc/pretraineds/pretraineds_custom/G48k.pth` and `/content/Applio/rvc/pretraineds/pretraineds_custom/D48k.pth`, make sure to select the sample rate according to the [sample rate](/docs/getting-started/audio-analizer) of the custom pretraineds.
</Step>
<Step>
## Kaggle
<Steps>
...
Please note that the space is limited to [Inference](/docs/getting-started/download-models) and [TTS](/docs/getting-started/tts) only, as it runs on a free CPU. The processing time is not optimal, and the longer the uploaded audio, the longer it will take. You can use the **split audio** option to help mitigate this.
</Step>
<Step>
## Paperspace
...
<Step>
You can [train](/getting-started/training) models while checking [Tensorboard](/docs/getting-started/tensorboard), do [inference](/docs/getting-started/download-models), or use [TTS](/docs/getting-started/tts).
...
- Then, in the Applio interface, input your **model name**, use the same **sample rate**, and proceed to the last part of the train tab. Set the same **[batch size](/docs/faq#terminology)**, **[pretrained](/docs/getting-started/pretrained#how-to-use-pretraineds) (if you used)** and increase the number of epochs you want to train.
