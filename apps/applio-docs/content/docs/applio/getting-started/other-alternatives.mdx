...
When inside execute the following **two cells**
    ...
    <ImageZoom src="/images/colab.png" alt="Applio Colab"  width={1000} height={500}  className="rounded shadow-xl shadow-black/10 dark:shadow-white/10 border border-white/10"/>
</Step>
...
- Paste the IP given on Collab.
    ...
    <ImageZoom src="/images/tunel.png" alt="Applio Colab Tunel"  width={1000} height={500}  className="rounded shadow-xl shadow-black/10 dark:shadow-white/10 border border-white/10"/>
...
<Accordions>
<Accordion title="Extra cells (only for training)">
...
- **Download all [custom pretrains](/applio/getting-started/pretrained.mdx):** Allows direct downloading of custom pretrains to Applio.
<ImageZoom src="/images/extra-colab.png" alt="Applio Colab Custom Pretrained"  width={1000} height={500}  className="rounded shadow-xl shadow-black/10 dark:shadow-white/10 border border-white/10"/>
...
- Then, in the Applio interface, input your **model name**, use the same **sample rate**, and proceed to the last part of the train tab. Set the same **[batch size](/faq#terminology)**, **[pretrained](/applio/getting-started/pretrained#how-to-use-pretraineds) (if you used)** and increase the number of epochs you want to train.
...
<Step>
## Applio no UI colab (recommended)
...
The notebook is easy to understand, even more so when this notebook shares the same processes as the UI version!, so you can be guided by the existing explanations such as [Training](/applio/getting-started/training) or [Inference.](/applio/getting-started/download-models)

Finally, if you had the **auto_backup** option enabled, your model will be saved to your Google Drive in case it needs to [resume training](/applio/getting-started/other-alternatives#how-to-resume-training-in-google-colab) or  upload or download files in Google Colab, click on the 📁 located in the left sidebar. This will open the file explorer, which allows you to transfer documents almost instantly between Google Drive and Colab. This feature facilitates file management, allowing you to quickly access and move data without hassle.

### How to use custom pretrained in Applio No UI?

- Download the [pretrained](/applio/getting-started/pretrained#where-to-find-pretraineds) you want to use and upload the d.pth and g.pth files to your Google Drive.
...
- Finally, copy the path of G.pth and paste it into **g_pretrained_path**, and do the same for D.pth into **d_pretrained_path**. It should look like this: `/content/Applio/rvc/pretraineds/pretraineds_custom/G48k.pth` and `/content/Applio/rvc/pretraineds/pretraineds_custom/D48k.pth`, make sure to select the sample rate according to the [sample rate](/applio/getting-started/audio-analyzer) of the custom pretraineds.
</Step>
<Step>
## Kaggle
<Steps>
...
<ImageZoom src="/images/duplicate.png" alt="Duplicate HF Space"  width={200} height={200}  className="rounded shadow-xl shadow-black/10 dark:shadow-white/10 border border-white/10"/>


Please note that the space is limited to [Inference](/applio/getting-started/download-models) and [TTS](/applio/getting-started/tts) only, as it runs on a free CPU. The processing time is not optimal, and the longer the uploaded audio, the longer it will take. You can use the **split audio** option to help mitigate this.
</Step>
<Step>
## Paperspace
...
<Step>
You can [train](/applio/getting-started/training) models while checking [Tensorboard](/applio/getting-started/tensorboard), do [inference](/applio/getting-started/download-models), or use [TTS](/applio/getting-started/tts).
...
- Then, in the Applio interface, input your **model name**, use the same **sample rate**, and proceed to the last part of the train tab. Set the same **[batch size](/applio/faq#terminology)**, **[pretrained](/applio/getting-started/pretrained#how-to-use-pretraineds) (if you used)** and increase the number of epochs you want to train.
