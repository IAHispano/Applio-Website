...
**1.** Enter the [Applio Colab](https://colab.research.google.com/github/iahispano/applio/blob/master/assets/Applio.ipynb) and execute the following **two cells**
    
    ![](/Colab.png)

 **2.** You will find that when the first **“Install Applio”** cell finishes executing the next one will give a URL, normally this is offered via gradio, but currently this tunnel disconnects the execution environment, it is recommended to activate the **“Share tunnel”** checkbox and follow the following simple procedure
...
Finally, for upload or download files in Google Colab, it is recommended to click on the folder located in the left sidebar. This will open the file explorer, which allows you to transfer documents almost instantly between Google Drive and Colab. This feature facilitates file management, allowing you to quickly access and move data without hassle.
...
Please note that the space is limited to inference only, as it runs on a free CPU. The processing time is not optimal, and the longer the uploaded audio, the longer it will take. You can use the 'split audio' option to help mitigate this.
