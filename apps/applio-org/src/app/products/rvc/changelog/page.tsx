	...
	return (
        <main className="mt-24 max-w-5xl flex flex-col justify-center mx-auto gap-8 max-lg:p-4">
            ...
            {!loading && Array.isArray(data) && ( 
                data.map((tag) => (
                    <a key={tag.id} href={`https://github.com/iahispano/Applio/releases/${tag.tag_name}`} rel="noreferrer" target="_blank" className="hover:bg-neutral-600/10 slow rounded-xl p-4 flex flex-col gap-2 border border-white/10">
                        <div className="flex justify-between items-start ml-auto w-full">
                        <h1 className="font-semibold text-3xl"><span className="max-md:hidden">Applio</span> <span className="read-font">{tag.tag_name}</span></h1>
                        <div className="flex flex-col md:gap-2 items-end">
                        <p className="md:text-sm text-xs text-neutral-300 read-font">{new Date(tag.published_at).toLocaleDateString("en-US", { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                        <p className="text-[10px] text-neutral-400 capitalize select-all">{tag.target_commitish} <span className="read-font">{tag.id}</span></p>
                        </div>
                        </div>
            ...
            {!loading && !Array.isArray(data) && <p className="text-xs text-center">No updates available.</p>}
            </div>
        </main>
    )
}
