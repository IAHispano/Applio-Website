...
export default function ApiUsage({ auth_id }: { auth_id: string | undefined }) {
    ...
    return (
        <section className="grid gap-4 w-full h-full">
            ...
            <>
            {data?.map((item: any) => (
                <div key={item.token} className="bg-white/10 md:h-[18svh] backdrop-blur-3xl rounded-xl p-4 border border-white/20 flex flex-col">
                    <div>
                        ...
                        <p className="text-3xl read-font font-medium tracking-thight text-[#9E9E9E] select-all mt-2 mb-3">{item.token}</p>
                    </div>
                    {item.usage < 100 && (<input type="range" max={100} min={0} value={item.usage} readOnly className="w-full accent-white focus:outline-none"/>)}
                    {item.usage > 100 && (<input type="range" max={100} min={0} value={item.usage} readOnly className="w-full accent-red-500 focus:outline-none"/>)}
