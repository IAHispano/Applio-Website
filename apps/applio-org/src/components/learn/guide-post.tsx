  ...
  return (
    <div className="text-black dark:text-white">
        {data?.length === 0 && !loading && (
            <h1 className="text-4xl text-center h-[80svh] mt-44">Oops... we didn&apos;t find that guide.</h1>
        )}
      {loading ? (
        ...
        data?.map((item) => (
            <article key={item.id} className="w-full h-full max-md:mt-12">
               <div className="absolute inset-0 h-full w-full overflow-visible opacity-20" style={{backgroundImage: "radial-gradient(100% 30% at 50% 0%, rgb(255, 255, 255), transparent)"}}/>
            ...
            <h1 className="text-4xl max-md:max-w-xs md:max-w-6xl text-wrap text-center truncate">{item.title}</h1>
            <h2 className="text-xl pl-0.5 mt-2 text-white/80">by {item.created_by} at {formatDate(item.created_at)}.</h2>
            </div>
            <div className="mt-28">
