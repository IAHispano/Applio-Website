    ...
    return (
      ...
      <div className="flex items-center justify-center p-6 h-[80svh]">
        <div className="w-full max-w-2xl md:h-fit h-full mt-44 md:m-52 bg-white/10 backdrop-blur-3xl md:p-12 max-md:p-4 rounded-lg">
          ...
          <button type="submit" onClick={handleSignUp}  className={`w-full p-3 font-bold rounded-xl bg-white text-black hover:bg-opacity-80 slow focus:outline-none border-2 border-white/20 ${success ? 'cursor-not-allowed opacity-40 hover:none' : 'cursor-pointer'}`} disabled={success}>Join the Applio family</button>
          {success && (
