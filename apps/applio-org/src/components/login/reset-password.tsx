        ...
        <section className="flex justify-center md:items-center m-auto w-full h-full p-8">
          <div className="bg-[#4d4c4c] w-full py-12 rounded-xl md:p-8 p-4 xl:max-w-[30%] md:max-w-[60%] md:max-h-fit flex flex-col gap-4">
            ...
            <div className="flex flex-col w-full h-full gap-4 md:mt-10 mt-6 max-md:px-2">
              <input value={email} onChange={(e) => setEmail(e.target.value)} className="rounded-xl bg-black/20 border border-white/20 p-3 focus:outline-none focus:border focus:border-white/40" placeholder="Email address"/>
              {error && <p className="flex justify-center text-xs p-3 bg-red-500/40 text-white rounded-xl font-medium">{error.message}</p>}
              {!success && !error && <button onClick={() => resetPassword(email)} className="w-full bg-[#666666] border border-white/20 hover:bg-opacity-80 hover:border-white/10 slow p-2 rounded-2xl font-semibold">Next</button>}
              {success && (
                <div>
                <div className="bg-white/20 rounded-xl p-2 text-sm text-center">We have sent you an email ✅</div>
