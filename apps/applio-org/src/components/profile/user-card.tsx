			...
			if (authData?.user) {
				...
				if (profileData) {
					const { data: followsData, error: followsError } = await supabase
						.from("follows")
						.select("*")
						.eq("by_id", profileData.id)
						.eq("follow_id", data?.id);
		...
		if (authData?.user) {
			if (userData && data) {
				if (follow) {
					...
					const removeFollow = await supabase
						.from("follows")
						.delete()
						.eq("by_id", userData.id)
						.eq("follow_id", data.id);
				} else {
					setFollow(true);
					if (followsCount != null) {
					...
					const sendFollow = await supabase.from("follows").insert({
						by: userData.full_name,
						by_id: userData.id,
						follow: data.full_name,
						follow_id: data.id,
					});
				}
			}
			...
			{data && !loading && (
				<>
					<div className="md:w-[70svh] md:h-fit w-full h-fit p-4 rounded-xl border border-white/20 bg-white/10 backdrop-blur-3xl flex flex-col md:mt-24 z-20">
						...
						<div className="md:justify-between w-full flex max-md:flex-col max-md:my-4 mt-4">
							<div
								className="flex justify-center items-center gap-2 my-auto max-md:mb-2 z-50 cursor-pointer hover:bg-white/10 p-1 rounded-xl"
							...
							>
								{data?.developer === true && (
									...
									>
										<path
											d="M1.8541 0L0 1.8541V10.1459L1.85413 12H10.1459L12 10.1459V1.8541L10.1459 0H1.8541ZM5.14365 9.31913H3.48413C3.48413 7.9464 2.36752 6.8298 0.994815 6.8298V5.1702C2.36752 5.1702 3.48413 4.05362 3.48413 2.68091H5.14365C5.14365 4.04282 4.47513 5.2431 3.45847 6C4.47513 6.75698 5.14365 7.9572 5.14365 9.31913ZM11 6.8298C9.6273 6.8298 8.5107 7.9464 8.5107 9.31913H6.8511C6.8511 7.9572 7.51972 6.75698 8.53635 6C7.51972 5.2431 6.8511 4.04282 6.8511 2.68091H8.5107C8.5107 4.05362 9.6273 5.1702 11 5.1702V6.8298Z"
											fill="white"
										/>
									</svg>
								)}
								...
								{data?.premium === true && (
									...
									>
										<path
											d="M14.1318 3.8822H0.131836L3.28109 0H10.9826L14.1318 3.8822Z"
											fill="#D7F8F9"
										/>
										<path
											d="M7.13183 11.5287L0.131836 3.88208H14.1318L7.13183 11.5287Z"
											fill="#6DBCDB"
										/>
										<path
											d="M4.41895 3.88208L7.13163 11.5287L9.80616 3.88208H4.41895Z"
											fill="#B3E2E5"
										/>
										<path
											d="M4.41895 3.8822L7.13163 0L9.80616 3.8822H4.41895Z"
											fill="#93D6D8"
										/>
										<path
											d="M4.41915 3.8822L3.28109 0L0.131836 3.8822H4.41915Z"
											fill="#B3E2E5"
										/>
										<path
											d="M9.80664 3.8822L10.9828 0L14.1321 3.8822"
											fill="#B3E2E5"
										/>
									</svg>
								)}
							...
							<div className="p-1">
								{userData && userData.full_name !== id && (
									<button
										className={`${follow ? "bg-white/20" : ""} border border-white/10 py-1 text-sm hover:bg-white/10 slow px-6 rounded-xl max-md:w-full`}
						...
						<div>
							<h1 className="text-4xl font-bold max-w-[200px] md:max-w-lg truncate">
								{data?.full_name}
							</h1>
							<h2 className="text-white/60 text-sm px-0.5 max-md:text-wrap">
								{data?.bio}
							</h2>
						</div>
					...
					{isOpen && (
						<motion.div
							...
							className="md:w-[70svh] h-fit bg-white/10 rounded-xl mt-4 flex flex-col p-4 z-50"
						>
							<p className="text-2xl font-semibold mb-4">
								How do the badges work?
							</p>
							<div className="flex flex-col gap-2 max-md:gap-6">
								<div className="flex max-md:flex-col gap-2 px-2">
									<svg
										width="20"
										height="20"
										viewBox="0 0 12 12"
									...
									<span>
										- You will earn this badge if you are a developer and have
										contributed to Applio.
									</span>
								</div>
								<div className="flex max-md:flex-col gap-2 px-2">
									<svg
									...
									<span>
										- You will get this badge if you have written at least one
										guide in Applio Guides.
									</span>
								</div>
								<div className="flex max-md:flex-col gap-1 px-2">
									<svg
										width="24"
										height="24"
										viewBox="0 0 15 12"
									...
									<span>
										- This badge is thanks to Applio Premium, when you buy one
										of our plans you will get it.
									</span>
								</div>
								<div className="flex max-md:flex-col gap-1 px-2">
									<svg
									...
									<span>
										- If you see this badge, you have just discovered an Applio
										administrator.
									</span>
								</div>
							</div>
