  ...
  async function logout() {
    ...
    return (
        ...
        {position === "top" && (
            ...
            <div className="z-50 w-full flex flex-cols-2 justify-between px-4 py-2">
              <div className="bg-white/10 backdrop-blur-3xl rounded-xl w-fit p-1 gap-2 h-[48px] justify-center items-center flex flex-cols mt-3 mr-5 cursor-pointer" onClick={getDropdown}>
                ...
                <p className="text-sm mr-1">{userData?.full_name || `${t("loading")}`}</p>
              </div>
        ...
        {dropdownVisible === true && (
          <div className="bg-black/20 backdrop-blur-3xl rounded-xl max-md:rounded-t-xl md:w-52 w-full h-[90svh] md:h-fit p-4 fixed z-50 md:top-5 top-28 md:left-36 shadow-3xl">
            <div className="flex flex-col gap-2 max-md:gap-4">
            <a href="/upload" className="bg-white/20 backdrop-blur-xl border border-white/20 w-full rounded-xl p-4">{t("upload_video")}</a>
            <a href="/settings" className="bg-black/10 backdrop-blur-xl border border-white/20 w-full rounded-xl p-4">{t("settings")}</a>
            <a href="https://github.com/iahispano" target="_blank" rel="notarget" className="bg-black/10 backdrop-blur-xl border border-white/20 w-full rounded-xl p-4">{t("report_bug")}</a>
            <button onClick={logout} className="bg-red-500/10 backdrop-blur-xl border border-white/20 w-full rounded-xl p-4 text-left">{t("logout")}</button>
            </div>
