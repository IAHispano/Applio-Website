...
import {Table, TableHeader, TableColumn, TableBody, TableRow, TableCell, Chip, Tooltip, ChipProps, getKeyValue, Input, Button, Spinner, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, useDisclosure, Checkbox, Link} from "@nextui-org/react"
import { createClientComponentClient } from "@supabase/auth-helpers-nextjs";
import { PostgrestError } from "@supabase/supabase-js";
import { LockIcon,  User, Pencil, SearchIcon, X, Shield, Activity } from "lucide-react";
...
export default function KeysTable({ id }: { id: string }) {
  ...
  async function fetchData() {
      ...
      if (error) {
          ...
          const promises = fetchedData.map(async (user: any) => {
              try {
                  ...
                  if (!profileError) {
                      return { user: user.user, fullName: profileData?.full_name || user.user };
                  } else {
    ...
    const openModal = useCallback((token: User) => {
    ...
    const renderCell = useCallback((user: User, columnKey: React.Key) => {
      ...
      switch (columnKey) {
        case "status":
          ...
          case "user":
            return (
              <div onClick={() => window.location.href = `/user/${fullNames[user.user] || user.user}`} className="cursor-pointer">
          ...
          return (
            <div className="relative flex items-center gap-2">
              ...
              <Tooltip color="danger" content="Delete">
                <span className="text-lg text-danger cursor-pointer active:opacity-50" onClick={() => handleDelete(user.token)}>
                  <X />
                </span>
              </Tooltip>
            </div>
    ...
    return (
        ...
        <div className="">
          <div className="md:mx-24 h-fit">
            ...
            <Table aria-label="Users table"
              bottomContent={hasMore && !dataLoaded ? (
            ...
            >
              ...
              <TableBody isLoading={dataLoaded} loadingContent={<div className="my-8"><Spinner /></div>}>
                {(posts ?? []).map((post: any, index: number) => {
                  ...
                  return (
                    <TableRow key={id}>
                      ...
                      <TableCell className="capitalize">{renderCell(post, "role")}</TableCell>
        ...
        >
          <ModalContent>
            {(onClose) => (
              <>
                <ModalHeader className="flex flex-col gap-1">Update key information
                <span className="text-sm text-default-400">{selectedToken?.token}</span>
                </ModalHeader>
                <ModalBody>
                  <Input
                    ...
                    label="User"
                    readOnly
                    defaultValue={(selectedToken?.user !== undefined && fullNames[selectedToken?.user]) || selectedToken?.user}
                    variant="bordered" />
                  <Input
                    endContent={<Shield className="text-2xl text-default-400 pointer-events-none flex-shrink-0" />}
                    label="Role"
                    ...
                    variant="bordered" />
                    <Input
                    endContent={<Activity className="text-2xl text-default-400 pointer-events-none flex-shrink-0" />}
                    label="Status"
