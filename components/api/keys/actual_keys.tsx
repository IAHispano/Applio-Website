  ...
  return (
    <div className="max-w-6xl w-full flex flex-col gap-5 justify-center items-center text-left md:py-5">
      <div className="flex items-center gap-2 w-full flex-wrap">
        <h1 className="text-2xl md:text-5xl font-bold tracking-tight flex-grow">
          Current keys
        </h1>
        <a
        ...
        ${
          userTokens.length < 3
            ? "hover:bg-white cursor-pointer active:opacity-50"
      ...
      {loading ? (
        ...
        <section className="w-full flex flex-col items-center justify-center gap-4">
          {userTokens.map((token: Token) => (
            <article
              key={token.id}
              className="w-full bg-black border-2 border-white/30 rounded-3xl"
            >
              <div className="flex flex-col text-left justify-start items-start gap-2 p-3">
                ...
                <p className="text-3xl p-1 whitespace-pre-line select-all blur-xl hover:blur-none font-bold gtransition">
                  {token.token}
                </p>
              </div>
            </article>
        ...
        <p className="text-lg text-neutral-500 mt-4">
          No tokens available. Click &quot;Create a key&quot; to generate one.
        </p>
      )}
    </div>
  )
