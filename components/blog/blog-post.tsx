...
export default function BlogPost({ id }: Readonly<{ id: string}>) {
      ...
      return (
        ...
        <div
            className="h-8 w-8 animate-spin rounded-full border-4 border-solid border-current border-r-transparent align-[-0.125em] text-success motion-reduce:animate-[spin_1.5s_linear_infinite]">
        </div>
    </div>
        ) : (
        data &&
          data?.map((item) => (
        <div key={id}><div className="dark:h-[48rem] absolute w-full pointer-events-none overflow-hidden">
                  <div className="h-96 top-0 absolute w-full scale-120 rounded-3xl blur-3xl filter brightness-90 pointer-events-none">
                      <img className="z-10 absolute top-0 left-0 w-full h-full object-cover object-center" src={item.image_url} alt="post banner">
                      </img>
                  </div>
              </div>
              ...
              <a className="bg-white text-black gtransition py-2 px-4 hover:opacity-50 rounded-lg border-2" href={`https://twitter.com/intent/tweet?text=${item.title} www.applio.org/blog/${item.id}`} target="_blank" rel="noreferrer">
                Share
              </a>
              </div>
              <div className="px-5 w-full flex justify-center items-center">
                <div className="pt-5 pb-10 z-10 flex justify-center items-center gap-5 flex-col w-full max-w-4xl">
                    <div className="overflow-hidden w-full max-w-md md:hover:scale-105 md:active:scale-150 md:active:z-50 rounded-2xl relative shadow-2xl mb-5 gtransition">
                        <img src={item.image_url} alt={item.title} className="object-fill h-full w-full"></img>
                    </div>
                    ...
                    <div className="flex flex-col-2 text-left gap-2 flex-grow justify-center items-center w-full">
                        <p className="text-xs md:text-lg tracking-tight dark:text-neutral-300 text-left">{`Written on ${formatDate(item.created_at)},`}</p>
            ...
            components={{
                ...
                h1: ({ node, children, ...props }) => (
                    <h1 {...props} className="text-4xl font-bold mb-8 dark:text-white">
                      {children}
                    </h1>
                  ),
                h2: ({ node, children, ...props }) => (
                <h2 {...props} className="text-3xl font-bold mb-4 dark:text-white">
                    {children}
                </h2>
                ),
