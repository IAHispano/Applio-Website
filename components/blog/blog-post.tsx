...
export default function BlogPost({ id }: Readonly<{ id: string}>) {
      ...
      return (
        ...
        data &&
          data?.map((item) => (
        <div key={id}><div className="h-[48rem] absolute w-full pointer-events-none overflow-hidden">
              ...
              <div className="px-5 w-full flex justify-center items-center">
                <div className="pt-5 pb-10 z-10 flex justify-center items-center gap-5 flex-col w-full max-w-4xl">
                    ...
                    <div className="flex flex-col-2 text-left gap-2 flex-grow justify-center items-center w-full">
                        <p className="text-xs md:text-lg tracking-tight text-neutral-300 text-left">{`Written on ${formatDate(item.created_at)},`}</p>
                        <p className="text-xs md:text-lg tracking-tight text-neutral-300 text-left">{`by ${item.by}.`}</p>
                    </div>
            ...
            components={{
                ...
                h1: ({ node, children, ...props }) => (
                    <h1 {...props} className="text-4xl font-bold mb-8 text-white">
                      {children}
                    </h1>
                  ),
                h2: ({ node, children, ...props }) => (
                <h2 {...props} className="text-3xl font-bold mb-4 text-white">
                    {children}
                </h2>
                ),
