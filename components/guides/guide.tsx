...
export default function Guide() {
    ...
    function formatRelativeDate(dateString: string | number | Date) {
      ...
      if (timeDifference < hourMillis) return `about ${Math.floor(timeDifference / minuteMillis)} minute${timeDifference < 120000 ? '' : 's'} ago`;
      if (timeDifference < dayMillis) return `about ${Math.floor(timeDifference / hourMillis)} hour${timeDifference < 7200000 ? '' : 's'} ago`;
    ...
    if (_error) {
      return (
        <p className="text-neutral-300 text-lg flex items-center justify-center" suppressHydrationWarning>No guides found.</p>
      )
    }
    ...
    {!loading && (
        ...
        >
            {posts?.map((post: any, index: number) => {
              const {
                id, created_at, full_name, type, title, description, image,

              } = post
              const formattedDate = created_at ? formatRelativeDate(created_at) : 'Unknown Date'
              const imageStyle = index === 0 ? { height: '220px' } : { height: '180px' }
                  ...
                  <section className="w-full flex flex-col items-center justify-center gap-4">
                    <a className="mx-12 lg:w-full hover:scale-[99%] flex flex-col text-left justify-center items-start gap-2 p-3  border border-white/30  bg-black bg-opacity-70 rounded-xl gtransition cursor-pointer " href={`/guides/${id}`} >
                      ...
                      <div className="flex justify-start w-full flex-wrap gap-2 items-center md:justify-end">
                        <a className="flex gap-2 items-center justify-center bg-neutral-900 hover:bg-neutral-800 active:opacity-50 rounded-lg p-2 gtransition max-md:w-full max-md:mt-2" href={`/user/${full_name}`}>
                          <p className="text-neutral-300 font-medium md:text-xs text-sm">by <span className="text-white flex-wrap truncate">{full_name || "Unknown User"}</span> {formattedDate}</p>
                        </a>
                      </div>
                    </a>
                  </section>
