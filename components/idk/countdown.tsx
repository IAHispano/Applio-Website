...
const CountdownTimer: React.FC = () => {
  const targetDate = new Date('2024-01-18T18:44:00Z');
    ...
    <div className={`text-3xl md:text-6xl py-10 text-white gtransition  z-50 ${isExpired ? 'expired' : ''}`} suppressHydrationWarning>
      {isExpired ? (
        <div>
          ...
          <p className='text-xl text-white/30 mt-4 tracking-tight'>Click to see the trailer</p>
        </div>
      ) : (
        <p suppressHydrationWarning className='md:tracking-tighter tracking-tight font-bold'>
          {timeRemaining.hours} hours, {timeRemaining.minutes} minutes, and {timeRemaining.seconds} seconds.
        </p>
      )}
    </div>
  );
