  ...
  return (
    ...
    >
      ...
      >
        {data.map((item: any) => (
          <a
            className="w-[300px] h-[300px] max-w-full relative rounded-2xl border flex-shrink-0 border-white/20 md:w-[450px] bg-white/10 backdrop-blur-sm cursor-pointer hover:opacity-80 gtransition z-50"
            key={item.id}
            href={`/models/${item.id}`}
          >
            {item.image_url === null ||
            ...
            imageError ? (
              ...
              <div>
                <object>
                  <img
                    className="absolute top-0 left-0 w-full h-full object-cover object-center rounded-2xl gtransition"
                    loading="lazy"
                    decoding="async"
                    data-nimg="fill"
                    src={
