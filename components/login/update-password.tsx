    ...
    return (
      ...
      <div className="flex items-center justify-center p-6 h-[80svh]">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} className="max-md:mt-12 z-[2] absolute border border-white/10 top-[50%] left-[50%] max-h-full h-full md:h-auto md:max-h-[85vh] w-full md:w-[90vw] md:max-w-[480px] translate-x-[-50%] translate-y-[-50%] md:rounded-xl bg-[#101010]  p-[25px] focus:outline-none ">
          ...
          <h1 className="text-xl ml-0.5 mt-2">Next, enter your new password.</h1>
          <input required type="password" name="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" className="my-4 w-full p-3 rounded-md bg-neutral-800 text-white placeholder-gray-500 focus:outline-none focus:border-white/40 focus:border-2" disabled={success}></input>
          <p className="text-red-500 font-mono text-sm text-center pb-2">{error}</p>
          <button onClick={handleUpdate} className="w-full p-3 font-bold rounded-md bg-neutral-800/80 text-white hover:bg-white/20  gtransition focus:outline-none border-2 border-white/20" disabled={success}>Next</button>
          {success && !error && (
          ...
          <motion.p initial={{opacity: 0}} animate={{opacity: 1}} transition={{duration: 0.8, delay: 1}} className="text-xs mt-2">Password changed.</motion.p>
          </div> 
          )}
