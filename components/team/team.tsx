    ...
    <div className="md:my-10 my-4 grid grid-cols-2 gap-x-5 gap-y-10 xs:grid-cols-3 sm:grid-cols-7">
      {data.map((item: any) => (
        <a className="flex flex-col" key={item.id} href={`/user/${item.full_name}`}>
          <img className="m-0 inline-flex rounded-md object-cover !xs:w-36 !xs:h-36 !sm:w-40 !sm:h-40 h-32 w-32 cursor-pointer gtransition" src={item.avatar_url || 'https://i.imgur.com/AIZdDXa.png'} alt={item.full_name} 
          onError={(e: React.SyntheticEvent<HTMLImageElement, Event>) => {
            e.currentTarget.src = 'https://i.imgur.com/AIZdDXa.png';
        }}/>
          <h3 className="mb-0 mt-4 text-white">{item.full_name}</h3>
        </a>
      ))}
    </div>
    <p className="md:text-sm text-xs text-gray-400 py-4">and all the awesome open source contributors at GitHub.</p>
